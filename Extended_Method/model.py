...
def linear_ramp(t, t0=0.0, dt=1.0, y0=0.0, dy=1.0, GS_slope = 0.0, GIS_slope=0.0):
    ...
    This function describes the piecewise linear ramp.
    ...
    dt : float
        Transition length
    y0 : float
        Function value immediately before the transition
    dy : float
...
def lnpost(theta, t, yobs):
    ...
    t0, y0, dy, GS_slope, GIS_slope = theta[[0, 2, 3, 4, 5]]
    dt, tau, sigma = np.exp(theta[[1, 6, 7]])
    # Priors go here
    # Additional terms are needed because dt and tau are
    # sampled as log-transformed variables
    # this makes sure that the priors are really the ones specified with
    # the respective distributions
    #
    # Start and stop of transition:
    tmin = np.min(t)   # earliest allowed transition time
    tmax = np.max(t)   # latest allowed transition time
    if t0 < tmin or t0 > tmax or (t0 + dt) < tmin or (t0 + dt) > tmax:
        return -np.inf
    # if dt is smaller than ~1 days
    if dt <= 0.0027:
        return -np.inf
    if sigma == 0 or sigma > np.abs(dy):
        return -np.inf
    if tau == 0:
        return -np.inf
    ...
    lnp += 1.0                                             # step height
    lnp += gamma_like(tau, 1.5, 0.05) + np.log(tau)        # autocorrelation time
    lnp += normal_like(GS_slope, 0.0, 0.1*np.abs(dy)/dt)   # Stadial slope
    lnp += normal_like(GIS_slope, 0.0, 0.1*np.abs(dy)/dt)  # Interstadial slope
    # Ramp function
    ypred = linear_ramp(t, t0, dt, y0, dy, GS_slope, GIS_slope)
    # likelihood function
...
def fit_rmse(t, y, p0=None):
    ...
    if p0 is None:
        p0 = (float(t[sort][int(len(t)/2)]), np.log(np.diff(t)[0]*len(t)/10.0), np.mean(y[sort][:10]),
              np.mean(y[sort][-10:]) - np.mean(y[sort][:10]), 0.0, 0.0)
...
def fit_map(t, y, theta0=None, **kwargs):
    ...
    theta_map : np.ndarray
        ...
        model (t0, ln(dt), y0, dy, GS_slope, GIS_slope, ln(sigma), ln(tau)).
    ...
    """
    if theta0 is None:
        t0, lndt, y0, dy, GS_slope, GIS_slope = fit_rmse(t, y)
        sigma = np.std(np.diff(y))
        theta0 = np.array((t0, lndt, y0, dy, GS_slope, GIS_slope, np.log(2.0), np.log(sigma)))
...
def fit_gridsearch(t, y):
    ...
    for i in range(len(tm)):
        ...
        for j in range(i + 1, len(tm)):
            ...
            ypred = linear_ramp(tm, t0, dt, y0, dy, GS_slope, GIS_slope)
            rms = np.sqrt(np.mean((ym - ypred) ** 2))
            if rms < rmsmin:
                pars = (t0, dt, y0, dy, GS_slope, GIS_slope)
                rmsmin = rms
    return pars
